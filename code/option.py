...
class Option:
    def __init__(self, screen: Screen,  controller: Controller, map: Map, language: str, save: Save, keylistener: KeyListener):
        ...
        self.image_background: pygame.Surface = None
    ...
    def draw(self):
        ...
        self.screen.full_background.blit(self.background, (0, self.screen.get_size()[1] - self.background.get_height()))
        
    
    def check_end(self):
        if self.keylistener.key_pressed(self.controller.get_key("escape")):
            self.player.menu_option = False
            self.keylistener.remove_keys(self.controller.get_key("escape"))
            return