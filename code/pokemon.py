...
class Pokemon:
    def __init__(self, data, level:int):
        ...
        self.level = level
        self.geneder = "female" if random.randint(1, 100) <= self.femaleRate else "male"
        if self.femaleRate == -1:
            self.geneder = "genderless"
        ...
        self.exp_to_nex_level = self.xp_to_next_level()
    ...
    def update_stats(self, stat):
        ...
        return ((((2 * base_stat + iv + (ev / 4)) * level) / 100 + level + 5) * nature)
        
        
    def xp_to_next_level(self):
        if self.level == 100:
            return 0
        if self.experienceType == 0: #courbe moyenne
            return self.level **3
        elif self.experienceType == 1: #courbe rapide
            return math.floor((4 * (self.level ** 3))/ 5)
        elif self.experienceType == 2: #courbe lente
            return math.floor((5 * (self.level ** 3)) / 4)  
        elif self.experienceType == 3: #courbe parabolique
            return math.floor(((6/5) * (self.level ** 3)) - (15 * (self.level ** 2)) + (100 * self.level) - 140)
        elif self.experienceType == 4: #courbe eratique
            if self.level <= 50:
                return math.floor((self.level ** 3)*((100 - self.level) / 50))
            elif self.level <= 68:
                return math.floor((self.level ** 3)*((150 - self.level) / 100))
            elif self.level <= 98:
                return math.floor((self.level ** 3)*((1911 - (10 * self.level)) / 3)/500)
            elif self.level <= 100:
